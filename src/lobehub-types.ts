 ...
 * $ pg-to-ts generate -c postgresql://username:password@db.biyvcvoxtvezcpkjidai.supabase.co:5432/postgres?sslmode=require -t accounts -t agent_bot_providers -t agent_cron_jobs -t agent_documents -t agent_eval_benchmarks -t agent_eval_datasets -t agent_eval_experiment_benchmarks -t agent_eval_experiments -t agent_eval_run_topics -t agent_eval_runs -t agent_eval_test_cases -t agent_operations -t agent_shares -t agent_skills -t agents -t agents_files -t agents_knowledge_bases -t agents_to_sessions -t ai_models -t ai_providers -t api_keys -t async_tasks -t auth_sessions -t briefs -t chat_groups -t chat_groups_agents -t chunks -t devices -t document_chunks -t document_histories -t document_shares -t documents -t embeddings -t file_chunks -t files -t files_to_sessions -t generation_batches -t generation_topics -t generations -t global_files -t knowledge_base_files -t knowledge_bases -t llm_generation_tracing -t message_chunks -t message_groups -t message_plugins -t message_queries -t message_query_chunks -t message_translates -t message_tts -t messages -t messages_files -t messenger_account_links -t messenger_installations -t nextauth_accounts -t nextauth_authenticators -t nextauth_sessions -t nextauth_verificationtokens -t notification_deliveries -t notifications -t oauth_handoffs -t oidc_access_tokens -t oidc_authorization_codes -t oidc_clients -t oidc_consents -t oidc_device_codes -t oidc_grants -t oidc_interactions -t oidc_refresh_tokens -t oidc_sessions -t passkey -t push_tokens -t rag_eval_dataset_records -t rag_eval_datasets -t rag_eval_evaluation_records -t rag_eval_evaluations -t rbac_permissions -t rbac_role_permissions -t rbac_roles -t rbac_user_roles -t session_groups -t sessions -t system_bot_providers -t task_comments -t task_dependencies -t task_documents -t task_topics -t tasks -t threads -t topic_documents -t topic_shares -t topics -t two_factor -t unstructured_chunks -t user_connector_tools -t user_connectors -t user_installed_plugins -t user_memories -t user_memories_activities -t user_memories_contexts -t user_memories_experiences -t user_memories_identities -t user_memories_preferences -t user_memory_persona_document_histories -t user_memory_persona_documents -t user_settings -t users -t verifications -t verify_check_results -t verify_criteria -t verify_rubric_criteria -t verify_rubrics -t workspace_audit_logs -t workspace_invitations -t workspace_members -t workspaces -s public
 *
 */
...
export interface NextauthAccounts {
  access_token: string | null;
...
export interface NextauthAccountsInput {
  access_token?: string | null;
  expires_at?: number | null;
  id_token?: string | null;
  provider: string;
...
const nextauth_accounts = {
  ...
  primaryKey: 'provider',
  foreignKeys: { user_id: { table: 'users', column: 'id', $type: null as unknown as Users }, },
  $type: null as unknown as NextauthAccounts,
  $input: null as unknown as NextauthAccountsInput
} as const;

// Table nextauth_authenticators
export interface NextauthAuthenticators {
  counter: number;
...
export interface NextauthAuthenticatorsInput {
  counter: number;
  credentialBackedUp: boolean;
  credentialDeviceType: string;
  credentialID: string;
...
const nextauth_authenticators = {
  ...
  primaryKey: 'user_id',
  foreignKeys: { user_id: { table: 'users', column: 'id', $type: null as unknown as Users }, },
  $type: null as unknown as NextauthAuthenticators,
  $input: null as unknown as NextauthAuthenticatorsInput
} as const;

// Table nextauth_sessions
export interface NextauthSessions {
  expires: string;
  sessionToken: string;
  user_id: string;
}
export interface NextauthSessionsInput {
  expires: string;
  sessionToken: string;
  user_id: string;
}
const nextauth_sessions = {
  ...
  foreignKeys: { user_id: { table: 'users', column: 'id', $type: null as unknown as Users }, },
  $type: null as unknown as NextauthSessions,
  $input: null as unknown as NextauthSessionsInput
} as const;

// Table nextauth_verificationtokens
export interface NextauthVerificationtokens {
  expires: string;
  identifier: string;
  token: string;
}
export interface NextauthVerificationtokensInput {
  expires: string;
  identifier: string;
  token: string;
}
const nextauth_verificationtokens = {
  ...
  foreignKeys: {},
  $type: null as unknown as NextauthVerificationtokens,
  $input: null as unknown as NextauthVerificationtokensInput
} as const;
...
const passkey = {
  ...
  $type: null as unknown as Passkey,
  $input: null as unknown as PasskeyInput
} as const;
...
const two_factor = {
  ...
  primaryKey: 'id',
  foreignKeys: { user_id: { table: 'users', column: 'id', $type: null as unknown as Users }, },
  $type: null as unknown as TwoFactor,
  $input: null as unknown as TwoFactorInput
} as const;
...
export interface TableTypes {
  ...
  nextauth_accounts: {
    select: NextauthAccounts;
    input: NextauthAccountsInput;
  };
  nextauth_authenticators: {
    select: NextauthAuthenticators;
    input: NextauthAuthenticatorsInput;
  };
  nextauth_sessions: {
    select: NextauthSessions;
    input: NextauthSessionsInput;
  };
  nextauth_verificationtokens: {
    select: NextauthVerificationtokens;
    input: NextauthVerificationtokensInput;
  };
